...
class BcosConv2d(nn.Module):
    ...
    def __init__(self, inc, outc, kernel_size=1, stride=1, padding=0, max_out=2, b=2,
                 scale=None, scale_fact=100,lin=False, **kwargs):
        super().__init__()
...
class Bcoslin(nn.Module):
    ...
    def __init__(self, inc, outc, kernel_size=1, stride=1, padding=0, max_out=2, b=2,
        ...
        ks = kernel_size
        self.stride = stride
        self.linear=nn.Linear(inc,outc* max_out)
    ...
    def fwd_b(self, in_tensor):
        ...
        bs, features = out.shape
        
        # Überprüfen, ob die Dimensionen korrekt sind
        if self.max_out > 1:
            # Reshape des Tensors, um ihn in Gruppen für die MaxOut-Berechnung aufzuteilen
            out = out.view(bs, -1, self.max_out)  # Umformen in (bs, num_groups, max_out)
            # Maximalwert entlang der letzten Dimension finden
            out = out.max(dim=2, keepdim=False)[0]  # max über max_out Dimension
